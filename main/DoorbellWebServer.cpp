...
constexpr const char* TAG = "doorbellweb";
...
DoorbellWebServer::DoorbellWebServer(WebContext* ctx, Settings& settings, AudioPlayer& player)
    : WebServer(ctx), settings_(settings), player_(player) {}

esp_err_t DoorbellWebServer::start() {
...
void DoorbellWebServer::populate_healthz_fields(WebContext*, JsonWrapper& json) {
...
// Deploy: curl --data-binary @build/doorbell3.bin http://<host>/firmware
esp_err_t DoorbellWebServer::firmware_post_handler(httpd_req_t* req) {
...
esp_err_t DoorbellWebServer::firmware_get_handler(httpd_req_t* req) {
    const esp_app_desc_t*  desc    = esp_app_get_description();
    const esp_partition_t* running = esp_ota_get_running_partition();
    JsonWrapper resp;
...
esp_err_t DoorbellWebServer::config_get_handler(httpd_req_t* req) {
...
// full settings. Volume changes take effect immediately; sensor_name/mqtt/tz
// changes take effect on the next reboot (they re-derive topics + hostname).
esp_err_t DoorbellWebServer::config_post_handler(httpd_req_t* req) {
    DoorbellWebServer* self = static_cast<DoorbellWebServer*>(req->user_ctx);
    ...
    auto changes = self->settings_.loadFromJson(json);
    self->settings_.save();
    for (auto c : changes) {
        if (c == Settings::Change::VolumeChanged) {
            self->player_.setDefaultVolume(self->settings_.volume);
        }
    }
    self->settings_.log();
...
esp_err_t DoorbellWebServer::config_reset_post_handler(httpd_req_t* req) {
    DoorbellWebServer* self = static_cast<DoorbellWebServer*>(req->user_ctx);
